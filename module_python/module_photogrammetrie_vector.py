...
def photogra_xm(Sx,Sy,Sz, M, o, p, k, f, cx, cy, k1, k2, k3, k4, p1,p2,b1,b2, w):
    S=jnp.array([
        Sx,
        Sy,
        Sz
...
def photogra_ym(Sx,Sy,Sz, M, o, p, k, f, cx, cy, k1, k2, k3, k4, p1,p2,b1,b2,h):
    S=jnp.array([
        Sx,
        Sy,
        Sz
...
def gauss_markov_non_lineaire_photog(uv,w, h, S, M, o, p, k, f, cx, cy, k1, k2, k3, k4, p1,p2,b1,b2, liste_inc=["S", "angles", "f", "k1"]):
    ...
    grad_xm_batched = vmap(grad_xm, in_axes=(None,None,None,0, None, None, None, None, None,None, None, None, None, None,None, None, None, None, None))

    

    grad_ym   = grad(photogra_ym, argnums=(num_param_inc)) 
    grad_ym_batched = vmap(grad_ym, in_axes=(None,None,None,0, None, None, None, None, None,None, None, None, None, None,None, None, None, None, None))
    
    
    Axm=grad_xm_batched(S_loc[0],S_loc[1],S_loc[2], M, o, p, k, f, cx, cy, k1, k2, k3, k4, p1,p2,b1,b2,w)
    Axm_numpy=np.array(Axm).T
    
    Aym=grad_ym_batched(S_loc[0],S_loc[1],S_loc[2], M, o, p, k, f, cx, cy, k1, k2, k3, k4, p1,p2,b1,b2,h)
    ...
    for i in range(nb_obs):
        l[i,0]=photogra_xm(S_loc[0],S_loc[1],S_loc[2], M[i,:], o, p, k, f, cx, cy, k1, k2, k3, k4, p1,p2,b1,b2,h)
        l[i+nb_obs,0]=photogra_ym(S_loc[0],S_loc[1],S_loc[2], M[i,:], o, p, k, f, cx, cy, k1, k2, k3, k4, p1,p2,b1,b2,w)
    ...
    for z in range(50):
        ...
        if np.max(np.abs(dx))<0.01:
            print(f"Convergence à l'itération {z}")
            break
        
        for i in range(nb_obs):
            l[i,0]=photogra_xm(Sx,Sy,Sz, M[i,:], o, p, k, f, cx, cy, k1, k2, k3, k4, p1,p2,b1,b2,w)
            l[i+nb_obs,0]=photogra_ym(Sx,Sy,Sz, M[i,:], o, p, k, f, cx, cy, k1, k2, k3, k4, p1,p2,b1,b2,h)
    ...
    #     # grad_xm_batched = vmap(grad_xm, in_axes=(None,None,None,0, None, None, None, None, None,None, None, None, None, None,None, None, None, None))
        ...
        Axm=grad_xm_batched(Sx, Sy, Sz, M, o, p, k, f, cx, cy, k1, k2, k3, k4, p1,p2,b1,b2,w)
        Axm_numpy=np.array(Axm).T
    ...
    #     # grad_ym   = grad(photogra_ym, argnums=(0,1,2,4,5,6,7,8,9,10,11,12,14,15)) 
        ...
        Aym=grad_ym_batched(Sx, Sy, Sz, M, o, p, k, f, cx, cy, k1, k2, k3, k4, p1,p2,b1,b2,h)
        Aym_numpy=np.array(Aym).T
    ...
    print(f"omega / phi / kappa={o*180/m.pi} / {p*180/m.pi} / {k*180/m.pi}")
    print(f"f={f}")
    print(f"cx={cx}")
    print(f"cy={cy}")
    print(f"k1={k1}")
    ...
    res ={
        ...
        "angles": [o*180/m.pi,p*180/m.pi,k*180/m.pi],
        "cx": cx,
        "cy": cy,
        "f": f,
        "f": f,
