...
@author: Bruno
"""
import module_python.module_photogrammetrie_vector as mpv
from module_python.lightglue import LightGlue, SuperPoint, DISK, SIFT, ALIKED, DoGHardNet
from module_python.lightglue.utils import load_image, rbd
from module_python.lightglue import viz2d
import torch
...
class calcul_orientation_photo_homol:
    def __init__(self, imagepath, position_approchee, projetcamera, pathprojet, show_plot=True):
        ...
        ----------
        imagepath : String
            nom de l'image (Exemple : DSC1955.JPG).
        position_approchee : np.array, shape(2,)
            coordonnée approchée de l'image utilisateur en 2D.
        projetcamera : class camera
            modèle photogrammétrique de la class camera.
        pathprojet : String
            Chemin du projet.
    ...
    def calcul_moindre_carre_non_lineaire(self, dict_homol=None, liste_key_none=[], liste_inc=["S", "angles", "f", "k1"]):
        ...
        return Qxx, x, A, B, wi, vi
    
    def calcul_dictkey_point_faux_selon_wi(self, B, wi, seuil_wi=3.0):
        """
        ...
        ----------
        B : array
            OBSERVATION DES MOINDRES CARRES.
        wi : array
            WI resutlat des moindres carrés.
        seuil_wi : float, optional
            seuil d'acceptation. The default is 3.0.
        ...
        for i in range(wi.shape[0]//2):
            ...
            if abs(wi[i, 0])>2.50 or abs(wi[i+nb_obs, 0])>2.5:
