...
class camera:
    ...
    def liste_coord_image_pt_homologue(self, pathlas, depthmap_IA, photoname, fact_reduce=0.1, calc_normal=False, inv=False):
        """
...
class depthmap:
    def __init__(self, pathdepthmap_IA, photoname, pathlas, calc_normal, camera,  max_megapixel=1.5, fact_reduce_IA=0.5, fact_reduce_depthajuste=0.1):
        """
    ...
    def transformation_simple_depthmap_IA(self):
        ...
        inc, vi, wi, B_calc,s0, Quot, Kxx=self.gauss_markov(Qll, A, B)
        max_depth=np.max(X)
    ...
    def calcul_transformation_cluster(self, cluster_array, cluster_index):
        ...
        Qll=1/sigma0**2*Kll
        inc, vi, wi, B_calc,s0, Quot, Kxx=self.gauss_markov(Qll, A, B, robuste=False, print_res=True)
    ...
    def gauss_markov(self,Qll, A, B,sigma0=1, robuste=False, iter_robuste=10, delta=2.5, print_res=False):
    ...
    def calcul_dist_ajustee(self, calcul_pt_homologue=True):
        ...
        if self.boundaries_proj is None:
            ...
            boundaries=self.boundaries_proj*self.fact_reduce_depthajuste
                    ...
                    if depth_IA_value_origine==0:
                        ...
                        if calcul_pt_homologue:
                            ...
                            if profondeur_valeur is not None:
                                ...
                                A[:,0]=X[:,0]
                                Qll=np.eye(nb_l)*0.1
                                inc, vi, wi, B_calc,s0, Quot, Kxx=self.gauss_markov(Qll, A, B, robuste=True, iter_robuste=4)
    ...
    def tranformation_depthanything_gauss(self, liste_prof, rob=4, nbre_inc=5):
        ...
        inc, vi, wi, B_calc,s0, Quot, Kxx=self.gauss_markov(Qll, A, B, robuste=False)
